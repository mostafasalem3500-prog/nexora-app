...
export default function Hero() {
  ...
  return (
    <section
      ...
      style={{
        background:
          "radial-gradient(ellipse 600px 400px at 85% 10%, rgba(42,92,255,.18), transparent 60%), radial-gradient(ellipse 500px 400px at 10% 90%, rgba(22,201,184,.10), transparent 60%), radial-gradient(ellipse 500px 500px at 50% 100%, rgba(124,92,255,.08), transparent 60%), var(--surface-bg)",
      }}
    >
      <div
      ...
      <div className="relative z-10 mx-auto max-w-[1240px] px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <motion.div custom={3} variants={fadeUp} initial="hidden" animate="show" className="flex gap-4 flex-wrap">
            ...
            <Link href="/work" className="rounded-md px-6 py-3 font-semibold border border-border-subtle-strong hover:border-brand-teal hover:text-brand-teal transition">
              {t("ctaSecondary")}
            </Link>
          </motion.div>
        </div>
        ...
        <div className="relative h-[340px] md:h-[440px]">
          <div className="glow-card absolute inset-inline-start-0 top-0 w-[80%] md:w-[66%] h-[320px] rounded-2xl border border-border-subtle bg-bg-deep-2 shadow-2xl overflow-hidden">
            <div className="h-6 flex items-center gap-1.5 px-2.5 bg-surface-hover">
              <span className="w-1.5 h-1.5 rounded-full bg-surface-active" />
              <span className="w-1.5 h-1.5 rounded-full bg-surface-active" />
              <span className="w-1.5 h-1.5 rounded-full bg-surface-active" />
            </div>
            <div className="p-4 font-en text-[.7rem] text-ink-muted">
              <div className="flex gap-2 mb-3">
                <div className="flex-1 bg-surface-hover rounded-lg px-2.5 py-2">
                  المبيعات
          ...
          <div className="glow-card absolute inset-inline-end-0 top-8 w-[55%] md:w-[44%] h-[200px] rounded-2xl border border-border-subtle bg-bg-deep-2 shadow-2xl overflow-hidden z-20">
            <div className="h-6 flex items-center gap-1.5 px-2.5 bg-surface-hover">
              <span className="w-1.5 h-1.5 rounded-full bg-surface-active" />
              <span className="w-1.5 h-1.5 rounded-full bg-surface-active" />
            </div>
          ...
          <div className="glow-card absolute inset-inline-end-[6%] bottom-[-10px] w-[110px] md:w-[150px] h-[220px] md:h-[300px] rounded-[26px] border-[6px] border-[#1a2438] bg-bg-deep-2 shadow-2xl overflow-hidden z-30">
            <div className="p-3 space-y-2 text-[.65rem]">
              ...
              <div className="bg-surface-hover rounded-lg p-2">تمت الموافقة على الفاتورة</div>
              <div className="bg-surface-hover rounded-lg p-2">تحديث لوحة القيادة</div>
            </div>
          </div>
        </div>
      </div>
